...
fun NativeAdCard(
    ...
    // The NativeAdView XML layout already includes its own background styling.
    // Wrapping it in a Compose Card with padding/shadows would place AdMob
    // asset boundaries outside the NativeAdView root, triggering the
    // "Advertiser assets outside native ad view" validation error.
    Box(
        modifier = modifier
            .fillMaxWidth()
